...
使用已有 SSH 连接及已核验的主机密钥，在客户端建立本地转发。示例中的 `HOST_ALIAS` 必须是实际可用的 SSH 配置别名；CardBush 的连接显示名称未必是系统 SSH 别名：

```sh
ssh -N -o ExitOnForwardFailure=yes -L 127.0.0.1:14780:127.0.0.1:4780 HOST_ALIAS
```

在 CardBush 中填写 `http://127.0.0.1:14780` 和该实例令牌。本地端口明确绑定回环地址；不要使用 `-g` 或绑定 `0.0.0.0`。隧道断开只影响接收结果，不会停止服务器任务；需要持续接入时另行托管隧道。此方式无需 Nginx、域名或 Agent 公网端口。
