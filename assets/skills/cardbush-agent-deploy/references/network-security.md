...
优先使用 CardBush 连接管理器托管：在 Agent 的连接设置中选择「SSH 直连（自动重连）」，关联已保存并核验主机指纹的 SSH 连接，填写服务器 Agent 端口（默认 `4780`）和访问令牌。HTTP 请求和事件流直接通过 SSH 通道送到服务器，无需填写本机地址或开启本机转发端口。连接管理器负责建立、重连和应用重启后的恢复；不要用 Agent 的临时终端维持长期隧道。
...
手动隧道使用 HTTP / HTTPS 连接方式填写 `http://127.0.0.1:14780` 和该实例令牌。此时本地端口明确绑定回环地址；不要使用 `-g` 或绑定 `0.0.0.0`。CardBush 内置 SSH 直连无需运行这条命令。隧道断开不停止服务器任务。此方式无需 Nginx、域名或 Agent 公网端口。
