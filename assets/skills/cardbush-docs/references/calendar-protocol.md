...
“日历数据”随应用离线携带「中国农历与节日」和「美国日历与节日」，两者默认关闭，可分别勾选启用，选择会保存在本机。启用中国日历同时显示农历日期和常见节日，不包含年度调休；美国日历包含全国联邦节日及标准周末补休。内置节日明细覆盖 2020–2050 年，可停用，不能移除或通过导入覆盖。

也可导入 UTF-8 JSON / ICS、停用或移除已导入日历。日历资料仅用于显示，不生成闹钟，也不向 agent 注入指令。实际执行任务见 [定时与自动化](automations.md)。移除导入数据不会删除源文件或自动化任务。
...
单文件上限 8 MiB；除两个内置日历外，最多保存 30 个导入数据集、合计 50,000 条导入条目，整体存储上限 16 MiB。内置日历的 `cardbush.chinese` 和 `cardbush.us` 是保留 ID，导入数据需使用其他 ID。解析在独立 Worker 中进行，5 秒超时，堆内存有上限；校验失败不会写入部分数据。
