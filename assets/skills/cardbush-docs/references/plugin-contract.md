...
预览、安装及运行使用同一个只读清单解析器。保留原始包文件，不生成转换清单或复制 Skills。Claude 包属于兼容入口，支持既有可加载组件；`strict: false` 的无独立清单包使用安装收据中的原始市场条目。安装收据保存获取来源与原始条目，不存一套改写后的能力定义。MCP 中 `${PLUGIN_ROOT}`、`${CLAUDE_PLUGIN_ROOT}`、`${CODEX_PLUGIN_ROOT}`、`${CARDBUSH_PLUGIN_ROOT}` 都在运行时解析为安装目录；`${PLUGIN_DATA}`、`${CLAUDE_PLUGIN_DATA}` 由宿主自动提供，与同一插件的 Hook 共用持久数据目录，启动前创建，更新插件时保留，卸载时清理。这些变量也注入 MCP 子进程和 HTTP 认证辅助进程。用户环境变量支持 `${NAME}` 和 `${NAME:-default}`，后者在变量未设置或为空时使用默认值。

缺失用户环境变量属于运行配置提示，不阻止安装文件。市场界面提供“安装，稍后配置”，安装后暂不启用；补齐变量后可以启用插件。结构不兼容、无有效启动命令、名称冲突等仍会阻止安装。启动时不会将未配置的必需变量静默替换为空字符串；未配置的可选 MCP 连接不影响其他连接，诊断接口报告具体变量名，声明为必需的连接仍要求先配置。
