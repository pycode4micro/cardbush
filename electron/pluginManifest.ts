...
export async function resolvePluginManifest(root: string) {
  ...
  const unsupported = componentKeys.filter(key => !['skills', 'mcpServers', 'hooks', 'agents', 'commands', 'apps'].includes(key) && manifest[key]);
  if (format === 'claude') {
    for (const directory of ['.lsp.json', 'monitors', 'themes', 'workflows', 'output-styles']) if (await exists(join(root, directory))) unsupported.push(directory);
  }
  ...
  return { root, manifestPath, manifest, format, skillRoots, registeredApps, authentication, extensions, issues, notes: extensions.notes };
}
