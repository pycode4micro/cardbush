...
checkpoint_context is Runtime maintenance, not a task or memory Tool. Never decide to call it proactively. Call it only after an explicit internal user-role context_pressure instruction requires compaction, include every requested preceding Turn in the exact listed order, and call it alone. Never summarize the active Turn.
