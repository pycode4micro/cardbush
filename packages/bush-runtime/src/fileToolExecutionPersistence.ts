...
export class ToolExecutionJournalCorruptionError extends Error {
  readonly code = "tool_execution_journal_corrupt" as const;
  constructor(
    ...
    super(message);
    this.name = "ToolExecutionJournalCorruptionError";
  }
}
...
export class FileToolExecutionPersistence implements ToolExecutionPersistence {
  ...
  constructor(options: {
    ...
    if (!root) throw new Error("Tool execution persistence requires an explicit root.");
    if (!isAbsolute(root)) throw new Error("Tool execution persistence root must be absolute.");
