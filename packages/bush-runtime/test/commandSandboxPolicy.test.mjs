...
test('host sandbox policy cannot be disabled or broadened by full control or turn metadata', () => {
  ...
  for (const env of [ { CARDBUSH_EXECUTION_SANDBOX: 'typo' }, { CARDBUSH_SANDBOX_NETWORK: 'maybe' },
    { CARDBUSH_SANDBOX_READ_ROOTS: '["relative"]' }, { CARDBUSH_SANDBOX_WRITE_ROOTS: '{}' } ]) {
...
test('registered terminals snapshot host policy and do not derive grants from the requested cwd', async () => {
  ...
  const call = context(); call.input = terminal.decodeInput({ command: 'irrelevant', cwd: process.cwd(), yield_time_ms: 1, shell: process.platform === 'win32' ? 'cmd' : 'posix', sandbox: false });
  await terminal.execute(call);
  assert.deepEqual(start.sandbox, { network: 'disabled', readableRoots: [], writableRoots: [resolve('project')] });
});
