...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await until('document.querySelector(".change-review-file-item.active")?.title==="C:/fixture/second.ts"');
    ...
    assert.equal(await read('document.querySelector(".change-review-dialog")===reviewNode && document.querySelector(".change-review-file-item.active").title==="C:/fixture/second.ts"'),true,'review selection survives history');
    await read('navigation.openReview("C:/fixture/first.ts"); void 0');
    await until('document.querySelector(".change-review-file-item.active").title==="C:/fixture/first.ts"');
    ...
    assert.equal(await read('document.querySelector(".change-review-file-item.active").title'),'C:/fixture/second.ts','a live report refresh must not reapply an old file request');
    await select('history:a');
    await read('navigation.openReview("C:/fixture/first.ts"); void 0');
    await until('document.querySelector(".change-review-file-item.active").title==="C:/fixture/first.ts"');
    ...
    assert.deepEqual(await read('Array.from(document.querySelectorAll(".right-inspector-tab-page.active .work-summary-inspector-turn"),node=>node.dataset.turnId)'),['turn-5'],'a single-turn request only renders that turn');
    assert.equal(await read('document.querySelector(".right-inspector-tab-page.active .work-summary-turn-selector")'),null,'the full-history picker is absent in a single-turn view');
    assert.equal(await read('document.querySelector(".right-inspector-tab-page.active .work-summary-turn-inspector").scrollTop'),0,'the selected turn starts at its own beginning');
    await read('window.singleHistoryNode=document.querySelector(".right-inspector-tab-page.active .work-summary-turn-inspector"); void 0');
    writeFileSync(resolve('tmp/inspector-single-turn.png'),(await window.webContents.capturePage()).toPNG());
    await read('navigation.openHistory("turn-3","b"); void 0');
    await until('document.querySelector(".right-inspector-tab-page.active .work-summary-inspector-turn")?.dataset.turnId==="turn-3"');
    assert.deepEqual(await read('Array.from(document.querySelectorAll(".right-inspector-tab-page.active .work-summary-inspector-turn"),node=>node.dataset.turnId)'),['turn-3'],'opening another row replaces the previous turn');
    await read('navigation.openHistory("missing-turn","b"); void 0');
    await until('!!document.querySelector(".right-inspector-tab-page.active .work-summary-inspector-empty")');
    assert.equal(await read('document.querySelectorAll(".right-inspector-tab-page.active .work-summary-inspector-turn").length'),0,'a missing turn never falls back to unrelated history');
    await read('navigation.openHistory(undefined,"b"); void 0');
    await until('document.querySelectorAll(".right-inspector-tab-page.active .work-summary-inspector-turn").length===9');
    assert.equal(await read('document.querySelector(".right-inspector-tab-page.active .work-summary-turn-inspector")===singleHistoryNode'),true,'single and all details reuse the same session tab');
    await click('.right-inspector-tab-page.active .work-summary-turn-selector > button');
    await until('document.querySelectorAll(".work-summary-turn-selector-menu button").length===9');
    await read('document.querySelectorAll(".work-summary-turn-selector-menu button")[3].click(); void 0');
    await until('!document.querySelector(".work-summary-turn-selector-menu")');
    assert.equal(await read('document.querySelectorAll(".right-inspector-tab-page.active .work-summary-inspector-turn").length'),9,'the all-details picker navigates within the complete history');
    await read('navigation.openHistory("turn-5","b"); navigation.openHistory("turn-3","b"); void 0');
    await until('document.querySelector(".right-inspector-tab-page.active .work-summary-inspector-turn")?.dataset.turnId==="turn-3"');
    assert.equal(await read('document.querySelectorAll(".right-inspector-tab-page.active .work-summary-inspector-turn").length'),1,'returning from all details restores single-turn isolation');
