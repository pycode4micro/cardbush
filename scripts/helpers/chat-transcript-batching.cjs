  ...
  await run(`
    ...
    window.batchState = { 'batch-session': [
      ...
      { id: 'batch-intro', assistantMessageId: 'batch-intro', role: 'assistant', content: 'The existing narration remains.', turnId: 'batch-turn',
        toolExecutions: Array.from({ length: 19 }, (_, i) => ({ id: 'batch-history-' + i,
          ...
          metadata: {}, durationMs: 10, createdAt: '2026-09-15T00:00:00Z', turnId: 'batch-turn',
    ...
    window.batchTool = (id, state = 'completed') => batchFlush(() => {
      batchState = views.appendToolExecution(batchState, 'batch-session', 'batch-intro', {
        ...
        metadata: {}, durationMs: 10, assistantMessageId: 'owner-' + id, turnId: 'batch-turn',
    ...
    window.batchObserver = new MutationObserver(() => {
      ...
      if (label !== batchObservations.at(-1)?.label) batchObservations.push({ label, at: performance.now() });
    });
    batchObserver.observe(document.querySelector('.message-list'), { subtree: true, childList: true, characterData: true });
  `);
  try {
    await until("batchLabel() === 'Processed 19 actions'", 'initial tool package');
    ...
    assert.equal(await run('batchLabel()'), 'Processed 19 actions', 'a short burst keeps the previous complete frame');
    await until("batchLabel() === 'Processed 20 actions'", 'latest burst state displayed');
    assert.deepEqual(await run('batchObservations.map(item => item.label)'), ['Processed 20 actions'], 'queued/running transients never flash between completed frames');
    ...
    await until("batchLabel() === 'Processed 32 actions'", 'continuous stream delivers its newest state');
    ...
    assert.equal(await run('batchLabel()'), 'Processing · 33 actions', 'Stop request immediately exposes the newest progress');
    await run(`
    ...
    `);
    assert.equal(await run('batchLabel()'), 'Processed 33 actions', 'a stopped turn immediately flushes the last tool result');
    await run(`
    ...
    assert.equal(await run('batchLabel()'), 'Awaiting solution selection', 'a required decision bypasses the display buffer');
