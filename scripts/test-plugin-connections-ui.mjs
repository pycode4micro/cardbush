...
window.troubleshootingReads=[];
Object.assign(window.cardbushDesktop,{
 pluginTroubleshootingContext:async(pluginId,componentId)=>{troubleshootingReads.push({pluginId,componentId});if(window.troubleshootingFailure)throw Error('PRIVATE_FAILED_CONTEXT');if(window.troubleshootingWait)await new Promise(resolve=>window.finishTroubleshooting=resolve);const item=fixtureApps.plugins.find(item=>item.id===pluginId);const pluginRoot='C:/Fixture Plugins/'+pluginId;return{capturedAt:'2026-09-15T08:00:00Z',application:'CardBush',pluginId,componentId,version:item.version,source:item.source,manifestPath:item.manifestPath,pluginRoot,pluginEnabled:true,serviceId:'plugin_'+pluginId.replaceAll('.','_')+'_'+componentId,pluginConfigurationRevision:fixtureApps.revision,mcpConfigurationRevision:fixtureOverview.revision,configuredLaunch:{transport:'stdio',command:'node',args:[pluginRoot+'/server.mjs'],cwd:pluginRoot,environmentNames:['API_KEY']}}},
...
const props={language:'zh',initialTab:'plugins',skills:[skill],disabledSkillNames:new Set(),onToggleSkill:()=>{},onReloadSkills:async()=>[skill],onLoadSkillDetail:async()=>skill,onOpenMcp:id=>opened.push(id??'new'),onNotify:message=>localNotifications.push(message)};
function PluginFixture(props){const [unified,setUnified]=React.useState(false);const [presentation,setPresentation]=React.useState('catalog');window.setUnifiedNetwork=setUnified;window.showNetworkCatalog=()=>setPresentation('catalog');return <PluginManagementPanel {...props} key={presentation} presentation={presentation} onOpenNetwork={unified?()=>{opened.push('proxy');setPresentation('network')}:undefined}/>}
