...
const fileStream = read('electron', 'localFileStream.ts');
...
assert.match(main, /localFileResponse\(normalizedPath, responseType, request, range \?\? undefined\)/);
assert.match(fileStream, /handle\.createReadStream\([\s\S]*?highWaterMark: 64 \* 1024/);
assert.match(fileStream, /'content-length': String\(length\)/);
