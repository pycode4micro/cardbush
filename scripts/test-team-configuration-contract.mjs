...
const productTeams = readFileSync(new URL('../packages/cardbush-team-plugin/ui/productTeams.ts', import.meta.url), 'utf8');
...
expect(
  app.includes("section === 'team'") && app.includes('<RuntimeDelegationSurface slot="sidebar"'),
...
expect(store.includes('fetchTeamWorkspace()') && store.includes('receipt.contentHash'), 'Team UI reads teams and profiles from one file receipt.');
expect(productTeams.includes("action: 'read', migration") && productTeams.includes('removeItem(teamsKey)'), 'Legacy configuration must migrate through the file store before removal.');
expect(store.includes('saveTeamWorkspace(') && store.includes('expectedHash: state.configurationHash'), 'Team save must persist the complete configuration with file conflict protection.');
expect(app.includes('selectedTeamId: backendCapabilities.teamMode ? teamWorkspace.selectedId'), 'Chat request context must receive the current Team selection.');
expect(app.includes("section === 'team'") && app.includes('teamWorkspace.title'), 'The Team page title must use the active Team name.');

const api = readFileSync(new URL('../packages/cardbush-team-plugin/ui/api.ts', import.meta.url), 'utf8');
...
expect(
  api.includes('readProductTeams()') &&
    ...
    api.includes('replaceProductTeamConfiguration(runtimeClient'),
...
expect(composer.includes('id: delegationCommand') && composer.includes('<ComposerTeamPicker'), '/team must open the frontend Team picker.');
