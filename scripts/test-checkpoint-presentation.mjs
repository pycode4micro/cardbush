...
const raw = [
  ...
  { role: 'developer', name: 'context_pressure', content: 'Runtime maintenance notice.' },
  { role: 'developer', name: 'context_compaction_correction', content: 'Retry maintenance.' },
...
const turn = { turnId: 't', turnSequence: 1, createdAt, completedAt: createdAt, status: 'completed', reason: 'model_response_completed', usage: {},
  messages: raw.map((message, index) => ({ messageId: `m${index}`, turnId: 't', turnSequence: 1, messageIndex: index, createdAt, message,
    ...([1, 2, 3, 4].includes(index) ? { metadata: { runtimeMaintenance: 'context_compaction' } } : {}) })) };
...
const visibilitySource = readFileSync('src/backend/runtimeMessageVisibility.ts', 'utf8');
const visibilityCode = ts.transpileModule(visibilitySource, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } });
const visibilityModule = { exports: {} };
vm.runInNewContext(visibilityCode.outputText, { module: visibilityModule, exports: visibilityModule.exports, Set });
const { isInternalRuntimeMessage } = visibilityModule.exports;
for (const name of ['context_pressure', 'context_compaction_correction']) {
  assert.equal(isInternalRuntimeMessage({ message: { role: 'developer', name, content: 'Maintenance.' } }), true);
  assert.equal(isInternalRuntimeMessage({ message: { role: 'user', name, visibility: 'internal', content: 'Old maintenance.' } }), true);
  assert.equal(isInternalRuntimeMessage({ message: { role: 'user', name, content: 'User-authored example.' } }), false);
}
