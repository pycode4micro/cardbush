...
const source = `import React from 'react';import{createRoot}from'react-dom/client';import{MessageBubble}from'${local('src/features/chatMessages/MessageBubble.tsx')}';import {MessageToolOutputs} from '${local('src/features/tools/MessageToolOutputs.tsx')}';import {ConversationHostContext} from '${local('src/features/conversationHost.ts')}';import '${local('src/styles/theme.css')}';import '${local('src/styles/app.css')}';
...
window.renderRemoteFixture=()=>root.render(<ConversationHostContext.Provider value={remoteHost}><div className="app theme-dark"><MessageToolOutputs sessionId="s" turnId="t" language="zh" executions={[execution]} artifacts={[{...image,path:'/srv/result.png'},{...file,path:'/srv/report.pdf'}]}/></div></ConversationHostContext.Provider>);
