...
const source = `import React from 'react';import{createRoot}from'react-dom/client';import{MessageBubble}from'${local('src/features/chatMessages/MessageBubble.tsx')}';import {MessageToolOutputs} from '${local('src/features/tools/MessageToolOutputs.tsx')}';import {McpAppPanel} from '${local('src/features/tools/McpAppPanel.tsx')}';import {ConversationHostContext} from '${local('src/features/conversationHost.ts')}';import '${local('src/styles/theme.css')}';import '${local('src/styles/app.css')}';
...
addEventListener('cardbush:mcp-app-message',e=>{window.followup=e.detail.text;window.followupHost=e.detail.hostId;e.detail.resolve()});
...
window.interfaceOverrides={};window.outputIds=null;window.openFailure=null;
...
window.renderFixture=active=>{const executions=mediaOnly?[{...files,artifacts:fixtureArtifacts??[fixtureImage]}]:outputIds?outputIds.map(id=>({...execution,id,turnId:interfaceOverrides[id]?.turnId??execution.turnId})):[{...execution,id:'old'},execution,files,edit,...(extraOutput?[{...execution,id:'new'}]:[])];root.render(<div className="app theme-dark" style={{overflow:'auto'}}><main style={{padding:24,maxWidth:800,margin:'auto'}}><div className={active ? "message-list-item streaming" : "message-list-item"}><MessageBubble message={{id:'message',conversationId:'s',turnId:'t',role:'assistant',content:fixtureContent??(mediaOnly?'图片已生成，预览在上方。点击可查看完整图片。':'设计已保存。[打开设计](cardbush-app:s/t/tool)，也可查看[此前设计](cardbush-app:s/t/old)。'),attachments:fixtureAttachments,status:active?'streaming':'completed',createdAt:'2026-09-10T00:00:00Z',toolExecutions:active?executions:[],loopHistory:active?[]:[{id:'earlier',role:'assistant',content:'Earlier tool output',createdAt:'2026-09-10T00:00:00Z',toolExecutions:executions}],metadata:{},...window.fixtureMessageOverrides}} language="zh" sending={active} activeTurnId={active?'t':''} activeAssistantMessageId={active?'message':''} onRegenerate={no} onEditUserMessage={no} onRetryGuidance={no} onRevertChangeReport={no} onOpenScene={no}/></div></main></div>)};
window.remoteCommands=[];window.remoteReads=[];window.openedRemoteFiles=[];
const remoteRuntime={dispose(){},client:{command:async(command,decode)=>{remoteCommands.push(command);return decode(await fixtureCommand(command.payload))}}};
const remoteHost={id:'cloud:s',sessionId:'s',runtime:remoteRuntime,plugins:[],pluginCommands:[],uploadFiles:async()=>[],toolDetails:async()=>[],openFile:path=>openedRemoteFiles.push(path),readFile:async path=>{remoteReads.push(path);return{name:'result.png',blob:await(await fetch(image.path)).blob()}}};
window.renderRemoteFixture=(active=false)=>root.render(<ConversationHostContext.Provider value={remoteHost}><div className="app theme-dark"><MessageBubble message={{id:'remote-message',conversationId:'s',turnId:'t',role:'assistant',status:active?'streaming':'completed',content:'云端结果。[云端设计](cardbush-app:s/t/tool)',toolExecutions:[execution]}} language="zh" sending={active} activeTurnId={active?'t':''} activeAssistantMessageId={active?'remote-message':''} onRegenerate={no} onEditUserMessage={no} onRetryGuidance={no} onRevertChangeReport={no} onOpenScene={no}/><MessageToolOutputs language="zh" artifacts={[{...image,path:'/srv/result.png'},{...file,path:'/srv/report.pdf'}]}/></div></ConversationHostContext.Provider>);
window.renderInlineFixture=()=>root.render(<div className="app theme-dark"><div className="message-list-item"><McpAppPanel sessionId="s" turnId="t" toolCallId="tool" language="zh" autoOpen /></div></div>);
...
try {
  const result = await build({ configFile: false, logLevel: 'silent', define: { 'process.env.NODE_ENV': '"production"' }, plugins: [{
    name: 'mcp-app-fixture', enforce: 'pre', resolveId(id, importer) { if (id.endsWith('__mcp_app_fixture__.tsx')) return '\0mcp-app-fixture.tsx'; if (id.includes('runtime-client/ElectronRuntimeSession') && /(?:conversationRuntime\.ts|fileMemo\.ts)$/.test(importer??'')) return '\0runtime-fixture'; },
    load(id) { if (id === '\0mcp-app-fixture.tsx') return source; if (id === '\0runtime-fixture') return 'export function createDesktopRuntimeSession(){return{dispose(){},client:{command:async({payload},decode)=>{window.localRuntimeCalls=(window.localRuntimeCalls??0)+1;return decode(await window.fixtureCommand(payload))}}}}'; },
