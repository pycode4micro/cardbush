...
本地插件 MCP 热更新同时比较连接参数和 `implementationFingerprint`。该指纹由插件版本、包内运行源码及依赖清单的内容生成，解决“清单和技能已更新，但相同启动参数仍复用旧进程”的问题。源码扫描不跟随符号链接，跳过依赖树、缓存、日志、输出、文档和技能目录；读取大文件时流式哈希，未变化文件复用哈希。只改时间戳、同内容重装、修改图标或文档不会重启 MCP。受影响插件的本地服务沿用活动任务结束后替换的生命周期，其他插件进程保留；新工具不改写正在执行的请求。

包外依赖的单独更新可通过管理工具 `reconnect_mcp_server`（准确的 `id`）复用宿主已有的单服务重连接口，不改配置、不重启应用。内置服务不接受这个管理操作。`pending` 表示需等活动任务结束，不能在同一轮不停轮询；服务健康、工具发现和实际业务可用性分别验证。`mcp_search` 的 `load`/`reload` 不会重启进程。
