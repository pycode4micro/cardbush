...
通过 SSH 隧道接入时，在 **连接设置 → 连接方式** 中选择 **SSH 隧道（自动连接）**，选择已保存的 SSH 连接，填写服务器上的 Agent 端口（默认 `4780`）和本机转发地址（例如 `http://127.0.0.1:14780`）。SSH 凭据和主机指纹复用设置中的 SSH 连接；首次使用新主机时需先核对并信任指纹。已有离线连接也可打开连接设置，启用隧道而不丢失 Agent 身份和历史。

CardBush 在连接 Agent 前建立仅监听本机回环地址的转发，断线后逐步延迟重连，并在重新启动应用时恢复已保存的隧道。健康检查只读取服务信息，不会重放任务提交或其他写操作。关闭应用会释放本机转发端口，远端 Agent 服务和任务继续运行。多个 Agent 使用不同本机端口；端口被旧隧道占用时会报错，不会借用未知监听进程。未启用此选项的 HTTP / HTTPS 连接继续直连，项目 SSH 连接也不会自行创建 Agent 隧道。
